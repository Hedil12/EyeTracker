    ...
    if len(eyes) == 2:
        ...
        for index, (x, y, w, h) in enumerate(eyes):
            ...
            if circle_params is not None:
                ...
                dilation_state = classify_pupil_dilation(ratio)  # New function call
                ratios.append(ratio)
                
                # Use dilation_state to overlay text or draw an indicator
                state_text = {1: "Dilated", 0: "Normal", -1: "Constricted"}
                
                cv2.putText(image, "Pupil state: " + state_text[dilation_state], 
                            (lab_x, lab_y + 30), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 255, 255), 2)
                
                # Optionally save dilation state in the log
                now = datetime.now()
                time_list.append(str(now))
                ratios_list.append((ratio, dilation_state))
...
def genFile(createFile, time_list, ratios_list, fileName=""):
    if createFile == 'y':
        ...
        newFile.write("DateTime,Ratio,State\n")
        
        for i in range(len(ratios_list)):
            ratio, dilation_state = ratios_list[i]
            newFile.write(f"{time_list[i]},{round(ratio, 3)},{dilation_state}\n")
